...
def feature_baseline(feature_data):
    """Iter-0 train_accuracy. Returns None if missing or sentinel zero (dspy)."""
    ph = feature_data.get("prompt_history", [])
    if not ph:
        return None
    val = _norm(ph[0].get("train_accuracy", 0.0))
    return val if val > 0 else None


def consensus_baselines(results, features, dataset_name=""):
    ...
    for feat in features:
        ...
        for name, data in results.items():
            if feat not in data["features"]:
                continue
            b = feature_baseline(data["features"][feat])
            if b is not None:
                per_method[name] = b
        if not per_method:
            continue
        ...
        if max(vals) - min(vals) > eps:
            ...
            warnings.warn(
                f"{prefix}Baseline accuracy differs across methods for feature '{feat}': {details}"
            )
...
def plot_per_dataset(dataset_name, results, features, baselines, tag):
    ...
    for ax, feat in zip(axes, features):
        ...
        for name, data in results.items():
            if feat not in data["features"]:
                continue
            ph = [h for h in data["features"][feat]["prompt_history"]
                  if "candidate_accuracy" in h]
            candidate = [_norm(h["candidate_accuracy"]) for h in ph]
            ...
            if candidate:
                ax.plot(x[1:], candidate, f"{style['marker']}--",
                        label=f"{name} candidate", color=style["color"], alpha=0.5, markersize=4)
            ax.plot(x, best, f"{style['marker']}-",
                    label=f"{name} best", color=style["color"], alpha=0.5, markersize=5)
    ...
    for name, curve in avg_curve(results, features, baselines).items():
        style = method_styles[name]
...
def plot_2x2(dataset_curves, ylabel, suptitle, out_name, y_range_fn):
    ...
    all_vals = np.concatenate(
        [c for ds in DATASETS for c in dataset_curves[ds].values()]
        + list(avg_of_avgs.values())
    )
    ...
    for ax, ds in zip(axes.flatten()[:3], DATASETS):
        for name, curve in dataset_curves[ds].items():
            ...
            ax.plot(np.arange(11), curve, f"{style['marker']}-",
                    label=name, color=style["color"], alpha=0.7, markersize=5)
...
def plot_combined(dataset_curves, tag):
    plot_2x2(
        dataset_curves,
        ylabel="Accuracy",
        suptitle="Per-dataset average accuracy and average-of-averages",
...
def main():
    ...
    for ds, results in results_by_ds.items():
        ...
        baselines = consensus_baselines(results, features, ds)

        plot_per_dataset(ds, results, features, baselines, tag)
        ...
        dataset_curves[ds] = avg_curve(results, features, baselines)
        dataset_length_curves[ds] = avg_length_curve(results, features)

    plot_combined(dataset_curves, tag)
