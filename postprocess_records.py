...
def main():
    ...
    # OCP (= our Contraceptives covariate) is its OWN cancer-specific category ("Oral contraceptive")
    # for its applicable cancers (breast/cervix/liver [risk] + uterus/ovary/colon/rectum [protective]).
    # One binary per applicable cancer = contraceptive use present pre-index; the Cox estimates the
    # (risk or protective) direction.
    ocp_slug = slugify("Oral contraceptive")
    ...
    for cancer in ocp_applicable_cancers(cancer_columns):
        out_col = f"cancerspecific__pre_index__{cancer}__{ocp_slug}"
        df[out_col] = contraceptives.map({True: "Yes", False: "No"})
        n_cs_cols += 1
